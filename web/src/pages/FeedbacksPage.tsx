...
export default function FeedbacksPage() {
  ...
  const load = async (p = 1) => {
    ...
    try {
      const res = await api.get('/admin/feedbacks', {
        params: { page: p, size, status: status || undefined, type: type || undefined, keyword: keyword || undefined }
      });
  ...
  return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 600, marginBottom: 16 }}>用户反馈</h2>
        ...
        feedbacks.map(f => (
          <div key={f.id} onClick={() => navigate(`/feedbacks/${f.id}`)} style={cardStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: '#999' }}>#{f.id} · 用户 #{f.userId}</span>
              <StatusBadge status={f.status} />
            </div>
            <p style={{ fontSize: 14, color: '#333', margin: '0 0 8px', lineHeight: 1.5 }}>
              {f.content}
            </p>
