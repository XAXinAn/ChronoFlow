...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<Void>> resetUserPassword(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponse.success("密码已重置为 SJL123", null));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> sendMessage(@RequestBody Map<String, Object> body) {
        ...
        Object receiver = body.get("receiverId");
        if ("all".equals(receiver)) {
            int count = messageService.sendToAllUsers(adminId, title, content);
            return ResponseEntity.ok(ApiResponse.success("已发送给全部 " + count + " 位用户", null));
        } else {
            Long receiverId = Long.valueOf(receiver.toString());
            messageService.sendFromAdmin(adminId, receiverId, title, content);
            return ResponseEntity.ok(ApiResponse.success("发送成功", null));
        }
    }

    @GetMapping("/messages")
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> listMessages(
            @RequestParam Long adminId) {
        return ResponseEntity.ok(ApiResponse.success("获取成功", messageService.getAdminMessages(adminId)));
    }

    @GetMapping("/messages/{id}")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getMessageDetail(
            @PathVariable Long id, @RequestParam Long adminId) {
        return ResponseEntity.ok(ApiResponse.success("获取成功", messageService.getMessageDetail(id, adminId)));
    }
}
