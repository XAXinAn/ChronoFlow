...
/**
 * ReadingAgent 单元测试 — 验证 PromptGuard 接入 + 事件类型。
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("ReadingAgent 测试")
class ReadingAgentTest {

    @Mock
    private SparkApiService sparkApiService;

    @InjectMocks
    private ReadingAgent agent;

    @BeforeEach
    void setUp() {
        when(sparkApiService.chatStream(anyString(), anyList()))
                .thenReturn(Flux.just("mocked content for ReadingAgent"));
    }

    @Test
    @DisplayName("getIntentLabel 应返回 ReadingAgent 的 intent")
    void getIntentLabel_shouldReturnCorrectLabel() {
        String expected = switch ("ReadingAgent") {
            case "QuizAgent" -> "QUIZ";
    ...
    void execute_normalInput_shouldEmitEvents() {
        ...
        StepVerifier.create(agent.execute(ctx))
                .assertNext(event -> {
                    ...
                    assertThat(event.getAgent()).isEqualTo("ReadingAgent");
                })
                .assertNext(event -> {
