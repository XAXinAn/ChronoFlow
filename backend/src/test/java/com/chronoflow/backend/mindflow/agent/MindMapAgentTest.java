...
/**
 * MindMapAgent 单元测试 — 验证 PromptGuard 接入 + 事件类型。
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("MindMapAgent 测试")
class MindMapAgentTest {

    @Mock
    private SparkApiService sparkApiService;

    @InjectMocks
    private MindMapAgent agent;

    @BeforeEach
    void setUp() {
        when(sparkApiService.chatStream(anyString(), anyList()))
                .thenReturn(Flux.just("mocked content for MindMapAgent"));
    }

    @Test
    @DisplayName("getIntentLabel 应返回 MindMapAgent 的 intent")
    void getIntentLabel_shouldReturnCorrectLabel() {
        String expected = switch ("MindMapAgent") {
            case "QuizAgent" -> "QUIZ";
    ...
    void execute_normalInput_shouldEmitEvents() {
        ...
        StepVerifier.create(agent.execute(ctx))
                .assertNext(event -> {
                    ...
                    assertThat(event.getAgent()).isEqualTo("MindMapAgent");
                })
                .assertNext(event -> {
