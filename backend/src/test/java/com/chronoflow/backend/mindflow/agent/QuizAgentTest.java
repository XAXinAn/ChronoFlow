...
/**
 * QuizAgent 单元测试 — 验证 PromptGuard 接入 + 事件类型。
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("QuizAgent 测试")
class QuizAgentTest {
    ...
    @InjectMocks
    private QuizAgent agent;

    @BeforeEach
    void setUp() {
        when(sparkApiService.chatStream(anyString(), anyList()))
                .thenReturn(Flux.just("mocked content for QuizAgent"));
    }

    @Test
    @DisplayName("getIntentLabel 应返回 QuizAgent 的 intent")
    void getIntentLabel_shouldReturnCorrectLabel() {
        String expected = switch ("QuizAgent") {
            case "QuizAgent" -> "QUIZ";
    ...
    void execute_normalInput_shouldEmitEvents() {
        ...
        StepVerifier.create(agent.execute(ctx))
                .assertNext(event -> {
                    ...
                    assertThat(event.getAgent()).isEqualTo("QuizAgent");
                })
                .assertNext(event -> {
