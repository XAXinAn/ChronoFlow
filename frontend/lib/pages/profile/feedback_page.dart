...
class _FeedbackPageState extends State<FeedbackPage> {
  ...
  Future<void> _pickImages() async {
    ...
    try {
      ...
      if (mounted) MessageUtils.showError(context, '图片选择失败: ${MessageUtils.cleanError(e)}');
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final uri = Uri.parse('${AppConstants.baseUrl}/feedback');
      ...
      if (response.statusCode == 200) {
        ...
        String msg = '提交失败';
        try { final body = jsonDecode(response.body); msg = body['message'] ?? msg; } catch (_) {}
        throw Exception(msg);
      }
    } catch (e) {
      if (!mounted) return;
      ...
      body: Column(children: [
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _buildSectionLabel('反馈类型'), const SizedBox(height: 12),
            _buildTypeSelector(), const SizedBox(height: 28),
            _buildSectionLabel('详细描述'), const SizedBox(height: 12),
            _buildContentInput(), const SizedBox(height: 28),
            _buildSectionLabel('上传截图（选填）'), const SizedBox(height: 12),
            _buildImageArea(),
          ]),
        )),
        _buildBottomButton(),
  ...
  Widget _buildTypeSelector() {
    return Row(children: _feedbackTypes.map((t) {
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(t['icon'] as IconData, size: 22, color: selected ? Colors.white : Colors.black54),
  ...
  Widget _buildContentInput() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: TextField(
          controller: _contentController, maxLines: 6, maxLength: _maxLength,
      ...
      Row(children: [
        if (length > 0 && length < _minLength)
          const Text('至少输入$_minLength字', style: TextStyle(color: Colors.red, fontSize: 12)),
        const Spacer(),
        Text('$length/$_maxLength', style: TextStyle(
          fontSize: 12, color: length > _maxLength * 0.9 ? Colors.red : Colors.black38,
        )),
      ]),
    ]);
  }
  ...
  Widget _buildImageArea() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_images.length < _maxImages)
        ...
        Container(
          padding: const EdgeInsets.symmetric(vertical: 10), width: double.infinity,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Flexible(child: Text('点击添加截图（最多$_maxImages张）',
            style: const TextStyle(color: Colors.black54, fontSize: 14), overflow: TextOverflow.ellipsis)),
        ]),
      ),
    );
  }
      ...
      child: SizedBox(width: double.infinity, height: 48,
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: Colors.black, disabledBackgroundColor: Colors.black12,
